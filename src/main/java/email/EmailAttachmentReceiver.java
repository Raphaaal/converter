	...
	public void downloadEmailAttachments(String host, String port, String userName, String password) {
		...
		try {
			...
			for (int i = 0; i < arrayMessages.length; i++) {
				// Treatment only for messages that are not deleted
				if(!arrayMessages[i].getFlags().contains(FLAGS.Flag.DELETED)) {
					...
					message.setFlag(FLAGS.Flag.DELETED, true); // Flag message that has been read so it will not be treated again
